...
function App() {
  ...
  const unstoppableLoginFunction = new UAuth({
    clientID: 'e1e48eb2-8e38-448b-887a-05d3cb4df358',
    scope: 'openid email wallet',
    redirectUri: 'http://localhost:3000',
  })

  const unstoppableDomainUserLogin = async () => {
    const user = await unstoppableLoginFunction.loginWithPopup()
    if (user) {
      localStorage.setItem('user', user.idToken.sub)
      localStorage.setItem('walletAddress', user.idToken.wallet_address)
      setuDUser(user.idToken.sub)
    ...
    <BrowserRouter>
      <div className="cl">
        <Navbar
          ...
          unstoppableDomainUserLogin={unstoppableDomainUserLogin}
